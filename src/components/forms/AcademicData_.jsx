...
//Constantes
import { Q01OPTIONS, Q03AOPTIONS, Q03BOPTIONS, Q09OPTIONS, COMPETENCIAS, COMPETENCIAS0 } from '../../utils/constantes';
...
const AcademicData = ({ handleChangeStep }) => {
  ...
  const handleChangeScore = (e) => {
    setStateq06({...stateq06, [e.target.name]:{cal: e.target.value}})
  }
  
  const handleAddSkill = (elem) => {
    console.log(elem);
    /*setLoading(false);
    const skillID = `q06a`;
    ...
    const object = {};
      ...
      setMapSkills([...mapSkills, {
        id: skillID,
        name: skill
      }]);
    setStateq06(...stateq06, object);
    setLoading(true);*/
  }

  const handleChangeSkill = (e) => {
    setSkill(e.target.value);
    const a = COMPETENCIAS0.filter((elem) => elem.name.startsWith(e.target.value));
    if(e.target.value !== ''){
      setSuggestionsSkills(a);
    } else {
  ...
  const Suggestions = () => {
    return (
        <div  className="w-full" style={{margin:"auto", textAlign:"initial", overflowY: "auto"}}>
            {
                suggestionsSkills.map((elem) => <div className="bg-slate-200 cursor-pointer p-2 border hover:bg-slate-300 hover:shadow-lg" key={elem.id} onClick={() => handleAddSkill(elem)}>{elem.name}</div>)
            }
        </div>
    );
  }
  ...
  return (
    ...
    <div className="p-6 rounded-lg shadow-lg bg-white lg:w-3/4 m-auto">
      ...
      <form>
        ...
        <div className="form-group mb-6">
          <label htmlFor="birthdate" className="block mb-2 text-lg font-medium text-gray-900 dark:text-gray-400 text-bold">Q02. ¿Cuál es tu fecha de nacimiento?</label>
        ...
        <div className="form-group mb-6">
          <label htmlFor="career" className="block mb-2 text-lg font-medium text-gray-900 dark:text-gray-400 text-bold">Q03.b ¿Cuál es tu carrera?</label>
        ...
        <div className="form-group mb-20 gap-2">
          <label htmlFor="degree" className="block mb-2 text-lg font-medium text-gray-900 dark:text-gray-400 text-bold">Q06 ¿Cómo calificaría su nivel de conocimiento o competencia (el que sea mayor) en lo siguiente? 1=nada, 2=poco, 3=moderado, 4=bastante alto, 5=muy alto</label>
          ...
          <input className="form-control block
            ...
            ease-in-out
            m-0
            focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none col-span-3 sm:col-span-4" type="text" placeholder="Ingresa Competencia" id="competencia" value={skill} onChange={(e) => handleChangeSkill(e)}/>
          <button type="button" className="
            ...
            duration-150
            ease-in-out
            col-span-2 sm:col-span-1" onClick={() => handleAddSkill()}>Agregar</button>
          ...
          </div>
          {
            /*mapSkills.map((c) => (
              ...
              <label htmlFor={c.id} className="ml-2 text-lg font-bold text-gray-900 dark:text-gray-300 col-span-4">{c.id} - {c.name}</label>
        ...
        <div className="form-group mb-10 gap-2">
          <label htmlFor="degree" className="block mb-2 text-lg font-medium text-gray-900 dark:text-gray-400 text-bold">Q09 Descríbete a ti mismo como generalmente eres ahora, no como deseas ser. Utilice la escala a continuación para calificar con qué precisión lo describe cada afirmación. 1=no me gusta nada, 2=no me parece mucho, 3=un poco como yo, 4=bastante como yo, 5=muy como yo</label>
          {
            Q09OPTIONS.map((c) => (
              <div className="items-center mb-4 grid grid-cols-6" key={c.id}> 
              <label htmlFor={c.id} className="ml-2 text-lg font-bold text-gray-900 dark:text-gray-300 col-span-4">{c.id} - {c.name}</label>
